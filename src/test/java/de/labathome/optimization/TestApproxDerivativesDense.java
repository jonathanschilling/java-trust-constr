...
class TestApproxDerivativesDense {
	...
	void testScalarScalar() {
		...
		// state across calls, leaking to any later FACTORY user.
		double jacDiff2 = NumDiff.approxDerivative(funScalarScalar, x0,
				new FiniteDifferenceOptions.FiniteDifferenceOptionsFactory()
						.method(FiniteDifferenceMethod.TWO_POINT).build());
		double jacDiff3 = NumDiff.approxDerivative(funScalarScalar, x0,
				new FiniteDifferenceOptions.FiniteDifferenceOptionsFactory().build());
		// TODO: COMPLEX_STEP

		double jacTrue = jacScalarScalar.applyAsDouble(x0);

		RelAbsAssertions.assertRelAbsEquals(jacTrue, jacDiff2, 1.0e-8);
		RelAbsAssertions.assertRelAbsEquals(jacTrue, jacDiff3, 1.0e-8);
	}
