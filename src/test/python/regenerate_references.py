...
def ineq_rosenbrock():
    """matlab-fmincon Rosenbrock with linear inequality x[0] + 2 x[1] <= 1."""
    rosen = lambda x: 100.0 * (x[1] - x[0] ** 2) ** 2 + (1 - x[0]) ** 2
    rosen_g = lambda x: np.array([
        -2 * (1 - x[0]) - 400 * x[0] * (x[1] - x[0] ** 2),
        200 * (x[1] - x[0] ** 2),
    ...
    constraint = LinearConstraint([[1.0, 2.0]], -np.inf, 1.0)
    x0 = np.array([-1.0, -0.5])
    result = minimize(rosen, x0, jac=rosen_g, hess=rosen_h, constraints=[constraint],
                      method="trust-constr", options={"xtol": 1e-10, "gtol": 1e-10})
    _print_result("ineq_rosenbrock", result)


def bounded_rosenbrock():
    """matlab-fmincon Rosenbrock with bounds: -2 <= x[0] <= 0, 0 <= x[1] <= 2."""
    from scipy.optimize import Bounds
    rosen = lambda x: 100.0 * (x[1] - x[0] ** 2) ** 2 + (1 - x[0]) ** 2
    rosen_g = lambda x: np.array([
        -2 * (1 - x[0]) - 400 * x[0] * (x[1] - x[0] ** 2),
    ...
    bounds = Bounds([-2.0, 0.0], [0.0, 2.0])
    x0 = np.array([-0.2, 0.2])
    result = minimize(rosen, x0, jac=rosen_g, hess=rosen_h, bounds=bounds,
                      method="trust-constr", options={"xtol": 1e-10, "gtol": 1e-10})
    _print_result("bounded_rosenbrock", result)
...
def elec_2():
    ...
    def fun(v):
        xc, yc, zc = coords(v)
        dx = xc[:, None] - xc; dy = yc[:, None] - yc; dz = zc[:, None] - zc
    ...
    def grad(v):
        ...
        gx = -np.sum(dx * dm3, axis=1)
        gy = -np.sum(dy * dm3, axis=1)
        gz = -np.sum(dz * dm3, axis=1)
        return np.hstack((gx, gy, gz))

    constraint = NonlinearConstraint(
        lambda v: np.array([v[i] ** 2 + v[n + i] ** 2 + v[2 * n + i] ** 2 - 1 for i in range(n)]),
        -np.inf, 0,
        jac=lambda v: np.hstack((2 * np.diag(v[:n]), 2 * np.diag(v[n:2*n]), 2 * np.diag(v[2*n:]))),
        hess=lambda v, lam: 2 * np.block([
            [np.diag(lam), np.zeros((n, n)), np.zeros((n, n))],
            [np.zeros((n, n)), np.diag(lam), np.zeros((n, n))],
            [np.zeros((n, n)), np.zeros((n, n)), np.diag(lam)]]),
    )
    result = minimize(fun, x0, jac=grad, hess="2-point", constraints=[constraint],
                      method="trust-constr", options={"xtol": 1e-10, "gtol": 1e-10})
    _print_result("elec_2", result)
